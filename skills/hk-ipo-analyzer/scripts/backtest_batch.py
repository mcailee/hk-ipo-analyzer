...
stocks = [
    {
        ...
        'has_greenshoe': False, 'is_18c': True,
        'hsi_1m': -5.7,
        'dark_price': 193.6, 'dark_high': 208.0,
        'actual_open_pct': 121.21, 'actual_close': 209.6, 'actual_high': 220.0,
    ...
    },
    {
        ...
        'has_greenshoe': False, 'is_18c': True,
        'hsi_1m': -5.7,
        'dark_price': 62.2, 'dark_high': 70.0,
        'actual_open_pct': 49.88, 'actual_close': 100.0, 'actual_high': 104.0,
    ...
    },
    {
        ...
        'has_greenshoe': True, 'is_18c': False,
        'hsi_1m': -5.7,
        'dark_price': None, 'dark_high': None,
        'actual_open_pct': -1.18, 'actual_close': 18.4, 'actual_high': 21.4,
    ...
    },
    {
        ...
        'has_greenshoe': True, 'is_18c': False,
        'hsi_1m': -5.7,
        'dark_price': 104.7, 'dark_high': 110.0,
        'actual_open_pct': 44.24, 'actual_close': 103.0, 'actual_high': 110.0,
    ...
    },
    {
        ...
        'hsi_1m': -5.7,
        'dark_price': None, 'dark_high': None,
        'actual_open_pct': None, 'actual_close': None, 'actual_high': None,
...
print('=' * 85)
print('港股新股 v4.0 回测分析 — 3月30日同批次5只新股')
print('=' * 85)
...
for s in stocks:
    ...
    results.append({
        ...
        'pred_up': prob.first_day_up_prob, 'pred_mid': prob.expected_return_mid,
        'pred_low': prob.expected_return_low, 'pred_high': prob.expected_return_high,
        ...
        'lot_size': s['lot_size'], 'offer_price': s['offer_price'],
        'actual_close': s['actual_close'],
    })

    status = '推迟上市' if s['actual_close'] is None else '已上市'
    print(f"\n{'─' * 85}")
    print(f"  {s['name']} ({s['code']}) | {s['industry']} | {status}")
    print(f"{'─' * 85}")
    cs_str = f"有({s['cs_ratio']}%)" if s['has_cornerstone'] else '无'
    gs_str = '有' if s['has_greenshoe'] else '无'
    print(f"  发行价 HK${s['offer_price']} | 超购 {s['sub_mult']:.0f}x | 基石 {cs_str} | 绿鞋 {gs_str}")
    print(f"  评分 {total_score:.1f} | 募资 {s['offer_size']/100:.1f}亿")
    print(f"  📊 预测: 上涨{prob.first_day_up_prob}% | {prob.expected_return_low:+.1f}%~{prob.expected_return_mid:+.1f}%~{prob.expected_return_high:+.1f}% | {prob.confidence_level}")
    print(f"  ⏱️ 策略: {sell.strategy}")

    if actual_d1 is not None:
        profit = (s['actual_close'] - s['offer_price']) * s['lot_size']
        dk_str = f"{dark_ret:+.1f}%" if dark_ret is not None else "N/A"
        print(f"  📈 实际: 暗盘{dk_str} → 首日{actual_d1:+.1f}% | 每手{'赚' if profit > 0 else '亏'} HK${abs(profit):,.0f}")
        print(f"  🎯 方向{'✅' if direction_ok else '❌'} | 区间{'✅' if in_range else '⚠️'}")
    else:
        print(f"  ⚠️ 推迟上市（3月27日宣布延期）")
...
print(f"已上市 {len(traded)}/5 | 方向准确 {d_ok}/{len(traded)} ({d_ok/len(traded)*100:.0f}%) | 区间命中 {i_ok}/{len(traded)} ({i_ok/len(traded)*100:.0f}%)")

errors = [(r['name'], r['actual_d1'] - r['pred_mid']) for r in traded]
for name, err in errors:
    print(f"  {name}: 偏差 {err:+.1f}pp {'(低估)' if err > 0 else '(高估)'}")
avg_err = sum(e for _, e in errors) / len(errors)
abs_err = sum(abs(e) for _, e in errors) / len(errors)
print(f"\n平均偏差: {avg_err:+.1f}pp | 平均绝对偏差: {abs_err:.1f}pp")
