...
**核心运行无需安装任何依赖。** 本 Skill 使用纯 Python 3 标准库（math/json/os/sys/argparse/subprocess），零外部依赖。

**可选增强：** 如安装了 Node.js（v18+），将自动通过 [skill:腾讯自选股数据工具]（westock-data）获取实时行情数据、恒指月度涨跌幅等，提升分析时效性。未安装时静默 fallback 到内置历史数据。

## 数据获取优先级

分析港股新股时，数据获取遵循以下优先级：

1. **优先使用 [skill:腾讯自选股数据工具]（westock-data）** 获取：
   - 实时/历史 K 线行情（`kline hkXXXXX day N`）
   - 恒指月度涨跌幅（`kline hkHSI month 24`）
   - 公司简况（`profile hkXXXXX`）
   - 港股资金流向（`hkfund hkXXXXX`）
   - 港股财务报表（`finance hkXXXXX`）
   - 港股新股日历（`ipo hk`）

2. **westock-data 无法提供的数据**（仍需 web_search/手动输入）：
   - IPO 超购倍数（subscription_mult）
   - 基石投资者信息（has_cornerstone）
   - 暗盘涨跌幅（dark_return）— 暗盘非交易所数据
   - 是否 18C 上市机制（is_18c）
   - 发行价

3. **fallback：内置 data.py 历史数据集**

### 数据更新命令

批量从 westock-data 拉取最新数据更新 data.py：
```bash
