...
description: "港股打新甜蜜区间分析器（V3.4精准匹配与暗盘联动版）。输入港股股票代码，自动判断已上市/未上市：已上市股票输出回测分析（所属区间定位、多因子评分、同类对比）；未上市新股输出卖出策略建议（暗盘/首日/3天/5天最优卖出时点）；不输入代码则生成全量回测报告。内置约128只港股新股历史数据集（2024H2~2026Q1），支持按市场状态（牛/熊/震荡）分别训练条件子模型。V3.4新增：18C/B类上市机制风险标记与评分惩罚、多维余弦相似度匹配（替代粗粒度区间匹配）、暗盘联动修正引擎（输入暗盘涨幅后修正首日/Day3/Day5预期）、SVG雷达图可视化。纯Python标准库，零外部依赖。"
---

# HK IPO Sweet Spot — 港股打新甜蜜区间分析器 V3.4

基于约128只港股新股历史数据的统计回测分析工具。通过多因子模型发现超购倍数的"甜蜜区间"，引入市场状态条件子模型（牛/熊/震荡）和多维相似度匹配，为打新散户提供数据驱动的自适应卖出策略建议。
...
### 6. 18C / B类上市机制分析（V3.4新增）
- 20只18C股票独立标记和统计
- 评分惩罚：无基石18C -8%，有基石18C -3%
- 全量报告：18C vs 非18C 对比分析板块
- 策略报告：18C 专项分析板块（含有/无基石内部对比）

### 7. 多维相似度匹配引擎（V3.4新增）
- 5维加权余弦相似度：超购35% / 基石20% / 行业15% / 募资15% / 18C 15%
- 替代V3.3的粗粒度区间匹配
- SVG雷达图可视化、相似度排名详情表
- 降级机制：匹配<3只时回退区间匹配
